...
import { TrendingUp, Target, CheckCircle, DollarSign, Award, Calendar, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
      <div className="border-b border-slate-200 px-4 py-3 flex items-center justify-between flex-wrap gap-3">
        ...
        {!isExpanded && (
          <div className="flex items-center gap-4 flex-wrap text-sm">
            <span className="text-slate-700"><strong>{formatPercent(kpis.closingRate)}</strong> Closing</span>
            <span className="text-slate-700"><strong>{kpis.clientsPaid}</strong> Encaissés</span>
            <span className="text-slate-700"><strong>{formatCurrency(kpis.avgAmount)}</strong> Panier</span>
      ...
      <div className="p-4 flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-6 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-slate-900">{formatPercent(kpis.closingRate)}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-xs text-slate-500">Panier moy.</span>
              <div className="text-lg font-bold text-slate-900">{formatCurrency(kpis.avgAmount)}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-xs text-slate-500">RDV/Client</span>
              <div className="text-lg font-bold text-slate-900">{kpis.rdvPerClient.toFixed(1)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-200 rounded-lg">
          ...
          <div>
            <span className="text-xs text-emerald-600 font-medium">Prime estimée</span>
            <div className="text-lg font-bold text-emerald-700">{formatCurrency(bonusEstimate.amount)}</div>
          </div>
        </div>
      </div>
      )}
